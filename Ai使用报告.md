...
### 学习方法
&emsp;&emsp;@author 巷北  
&emsp;&emsp;@time 2026-03-01 16:23:57  

&emsp;&emsp;上面的内容一直也在回看分析着, 但似乎仍不是最优解. 最近参考各种资料, 整理了一套可行性的学习方法论. <br>
&emsp;&emsp;先来说说上面的问题吧. 借助`ai`理解, 没有问题, 但理解了并不代表真的会了, 真的记住了. 以现在的角度来看, `ai`只是我学习的输入层面, 想要切实掌握与学会, 输出是必不可少的. 这里有一个721准则, 其中10%是输入, 70%是输出, 剩余的20%则是与他人分析交流. 这里的输入代表的是什么? 看书, 看视频, 问`ai`等等各种可利用的学习资源. 输出是什么? 刻意练习, 主动回忆. 所以, 这其实一针见血地指出了我上面疑问(为什么靠`ai`理解了, 编写`cmake`等代码的时候, 必须参考其它代码), 因为靠`ai`理解, 只是输入的一部分, 而最关键的输出, 可以说基本没有做.<br>
&emsp;&emsp;那么怎么切实地, 真真正正地掌握呢? 按照以下方法论, 坚持下去, 一定能真正地掌握的. 在此之前, 先说一下参考资料.<br>
&emsp;&emsp;书籍：《上瘾式学习》《西蒙学习法》《刻意练习》<br>
&emsp;&emsp;学习思想：费曼学习法 西蒙学习法 <br>
&emsp;&emsp;关键性资料似乎就这些, 但是还有很多相关的视频没有放出来.<br>
&emsp;&emsp;好了废话不多说, 先来看看方法论<br>
...
- 量化阶段有两点
    - 限时, 量化自己的学习内容
    - 根据721准则, 将自己的学习时间也量化出来
- 学习阶段比较杂乱
    - 遵循721法则
        - 721法则是什么呢? 10%的输入, 70%的输出, 20%与他人交流
        - 输入包括视频, 数据, ai等各种各样的学习资料.
        - 输出则是向外输出, 当我们学习新的知识之后, 需要反复练习, 这是一个输出过程. 
        - 输出的关键性思想是无保留, 练习完即删. 在上瘾式学习中有具体描述及原因.
    - 刻意练习, `anki`卡片, 心理表征
        - 我是用anki卡 来复习巩固的. 每个`anki`卡片, 对我来说是一个心理表征, 具体问题关键时刻优先想到的还是这个东西. 这里就说一下`anki`卡片的制作与复习吧.
            - 职责单一. 比如我想弄算法的`anki`卡片, 但是我发现, 在语言层面存在问题, `stl`也有问题. 但是要注意, 这并不是算法问题, 可以单独剖析出去, 专门学习或者专门制`anki`卡.
            - 任意修改. `anki`卡片制作完成后, 并不意味着不可修改. 后续随着理解的改变, 可能会有不同的见解, 或许也已经牢牢掌握, 可以简化一下内容.
            - 养成习惯. 一旦开始, 不要停下来.
            - 不要记录例子!!! 为什么呢? 例子没用.
            - 主动回忆.这是很关键的. 主动回忆, 其实就是上面说的输出的一部分. 卡片中需要记录关键记忆点, 而不是各种杂七杂八的解释, 分析, 总结, 这些没有用. 当看到卡片时, 能主动回忆起来的关键记忆点, 才是最重要的.
            - 卡片之间的组块联系.
    - 理解优先, 将自己当做老师, 如何教会自己的学生.
    - 每周, 将组块(`anki`卡片)压缩成一张纸的内容, 每月, 将`4`周的内容压缩成一张纸.
- 注意事项如下
    - 提高学习效率, 有效学习时间在`4`-`6`小时.
    - 想玩手机时, 就再认真学`10min`, `10min`之后如果还想玩, 那就玩.
    - 不想学习时, 就认真学`10min`, `10min`之后, 如果仍不想学, 就不学.
    - 可视化图表分析.
    - 无用论. 这个是指, 当我们深入进去的时候, 会感到这么做没有什么用. 跳出来看, 如果切实按照上面做的话, 必然有用.


关于这个学习方法论, 我也弄成了`anki`卡片, 上面几乎都是按照我所强调的, 单独写出来的. 单独总结, 没有任何用, 内化为自己的, 才是有效的. 另外, 我为什么一定要记住呢? 因为深入的时候, 总会存在各种问题, 而上述, 几乎给出面对每一个问题的应对策略. 如果不记住, 模糊地知道, 那么之后再遇到类似问题的时候, 仍然很模糊, 找不到应对方法, 最后又会消极一段时间.

举几个例子, 比如我想学习新的东西, 那么我肯定先分析限定时间, 以及量化学习内容. 这样可以有目标, 针对性强, 而不是一个虚无的目标, 比如: 我要学会`c++`, 无限时间的话, 你会`push`自己去学吗? 当制作`anki`卡片的时候, 我就会针对上述策略, 不记例子. 为什么不记录例子呢? 因为例子并不会让我主动回忆, 过了一眼例子, 可能会觉得自己会了, 但实际是一种欺骗. 记录各种例子, 不如将一个例子作为关键性记忆(心理表征)记在心中, 当看到类似问题的时候会优先考虑这一个例子, 并引发其他的思考. 等等等等.

对了, 最近又有了一个关键性的见解, 记录一下吧. 
每一本书, 都有着作者独特的理解. 我们去看一本书, 去学一本书, 实际上是看作者的理解与认识. 我们去背一本书, 相当于去背作者的理解. 背过作者的理解, 对个人而言有什么用吗? 没有用, 我觉得就是死记硬背, 而且我确实也做不到. 正确的做法我觉得是将作者的理解与自己的理解建立一种映射关系, 自己的理解很清晰, 很明确, 这才是关键. 一味地记忆作者的思想, 我觉得没太大用处, 终归不是自己的.
